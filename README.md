...
1. **Imitation is real, small, and lives on function words.** Across eight LLMs (Claude, GPT, and local open-weight models) prompted to imitate fifteen novelists, naming the author roughly triples envelope entry on the function-word layer — and still, most imitation output never enters the target author's own range of variation.
...
4. **Folk "AI tells" are a witch hunt, quantified.** We operationalized twelve popular tells — the em dash, "not X, but Y," rule-of-three triads, "delve," corporate jargon, hedging — and scored celebrated novelists against AI long-form fiction. The combined score is a coin flip (AUC ≈ 0.51). A threshold tuned to catch half the machine samples falsely flags about half the passages by Morrison, Ishiguro, and Pynchon. Four tells run *backwards* — the novelists use more exclamation marks, superlatives, and hedges than the machines do. The em dash, the best single tell, turns out to be a model-family variable, not an AI constant.
5. **Models are nobody's copy — and somebody anyway.** Unprompted model output lands outside every shelf author's envelope; it is, statistically, no one's prose. Yet the models identify as themselves at 97.8% against a 12.5% chance rate. Each model is its own stable, identifiable voice that is not any human's voice.
...
I also stress-tested the original instrument end-to-end on a full novel I wrote — the hardest test I could give the method, and the personal stake that keeps the research honest: the goal was never "make AI signals go away," it was *I authored the final language; it reads like me.*

## Provenance and authorship

I'm the architect: the questions Mnemosyne asks, its design, the judgment about what counts as voice, and the standard it's held to are mine. I built the implementation the way most software is built now — working alongside AI, directing LLM collaborators (Claude among them) through the code, and then commissioning those same classes of models to attack the results. I'd rather say that plainly than imply I hand-wrote a system that itself lives on the seam between human and machine writing. A study of machine imitation, built in collaboration with the machines it studies, has an obvious conflict to manage — which is exactly why the methodology leans on pre-specified gates, multi-vendor models, adversarial review, and a replication path that doesn't require trusting me. The design and the judgment are mine; the building was a collaboration; the evidence doesn't care either way.
